...
if [ "$command" = "api-server" ]; then
  ...
  LOG_FILE="-Dlogging.config=conf/master_logback.xml -Dspring.config.location=conf/application_master.properties -Ddruid.mysql.usePingMethod=false"
  CLASS=cn.escheduler.server.master.MasterServer
elif [ "$command" = "worker-server" ]; then
  LOG_FILE="-Dlogging.config=conf/worker_logback.xml  -Dspring.config.location=conf/application_worker.properties -Ddruid.mysql.usePingMethod=false"
